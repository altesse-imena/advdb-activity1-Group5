...
We started from the scenario's core operations — managing members, trainers, classes, and payments — and identified the nouns that needed to persist data as candidate entities. We then walked through the day-to-day questions the gym would need to answer (Who can lead this class? What times does it run? Who is enrolled? Has this member paid?) to surface the relationships and constraints between entities. Attributes were assigned to whichever entity they functionally depend on, primary keys were chosen as single-column surrogate identifiers for stability, and foreign keys were added everywhere a relationship needed to be enforced. The model was then normalized to third normal form (3NF) to remove repeating groups and update anomalies (see Section 3). The final logical model was drawn as an Enhanced ER Diagram (EERD) in Graphviz using crow's-foot cardinality notation, an ISA triangle for the trainer specialization, and double-bordered boxes for the two weak/associative entities.
...
**Additional supporting entities:** MembershipType (plan pricing/duration), Room (facility used to host a class session), ClassSchedule (a specific day/time/room a class runs — weak entity), Enrollment (associative entity resolving the Member–ClassSchedule many-to-many relationship), and PaymentMethod (separated out to satisfy 3NF, see Section 3).
...
3. A class can run as many separate scheduled sessions (different days, times, and rooms); each schedule entry belongs to exactly one class and cannot exist without it, making ClassSchedule a weak entity identified by (ClassID, ScheduleID).
...
- **MembershipType separated from Member (2NF/3NF).** Plan price and duration were originally copied onto every Member row. Since Price and DurationMonths depend on the plan, not on the individual member, leaving them on Member created a transitive dependency and an update anomaly (changing a plan's price would require updating every member on that plan). We moved them into their own MembershipType entity.
...
- **Member–ClassSchedule many-to-many resolved with an associative entity.** Because the relationship itself carries data (EnrollmentDate, AttendanceStatus), it cannot be represented as a plain M:N line; we created the Enrollment entity with a composite key (MemberID, ScheduleID).
...
Notation: rectangles are entities (primary keys underlined, foreign keys marked “FK”); double-bordered rectangles are weak/associative entities; crow's-foot line endings show cardinality (a fanned-out end = “many”, a perpendicular bar = “one”); the triangle labelled “ISA” shows the disjoint, partial specialization of Trainer into PersonalTrainer and GroupInstructor.
...
| **ClassSchedule** | ScheduleID (PK) | Unique identifier for one scheduled session (weak entity). | Integer | Auto-increment; not null |
| **ClassSchedule** | ClassID (FK) | Class this session belongs to (partial key). | Integer | Not null; must exist in Class |
| **ClassSchedule** | RoomID (FK) | Room the session is held in. | Integer | Not null; must exist in Room |
| **ClassSchedule** | DayOfWeek | Day the session runs. | Varchar(10) | Not null; one of Mon–Sun |
| **ClassSchedule** | StartTime | Session start time. | Time | Not null |
| **ClassSchedule** | EndTime | Session end time. | Time | Not null; must be after StartTime |
| **Enrollment** | MemberID (PK, FK) | Member enrolled in the session. | Integer | Not null; must exist in Member |
| **Enrollment** | ScheduleID (PK, FK) | Scheduled session the member enrolled in. | Integer | Not null; must exist in ClassSchedule |
...
| **Noah Jobse** | Documentation & Business Rules | [Add specific tasks – e.g., drafted business rules, wrote/reviewed the README design rationale, reviewed relationship cardinalities] |
| **Melody Shih** | Data Dictionary & QA | [Add specific tasks – e.g., compiled the data dictionary, validated attribute data types and rules, proofread the final report] |

*Note: update the bracketed task descriptions above to reflect what Noah and Melody actually worked on before submitting.*
